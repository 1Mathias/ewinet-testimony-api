...
router.post("/", async (req, res) => {
    const newHash = new HashTags(req.body);
    try {
        const savedHashtags = await newHash.save();
        res.status(200).json(savedHashtags);
    } catch (err) {
...
router.put("/:id", async (req, res) => {
    try {
        const category = await HashTags.findById(req.params.id);
